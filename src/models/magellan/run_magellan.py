...
def run_magellan(train_set, valid_set, test_set, feature_combinations, classifiers, experiment_name, dataset_name,
    ...
    os.makedirs(os.path.dirname(f'../../../reports/{dataset_name}/magellan/{experiment_name}/'),
                exist_ok=True)

    os.makedirs(os.path.dirname(f'../../../models/{dataset_name}/magellan/{experiment_name}/'),
                exist_ok=True)

    with open(f'../../../reports/{dataset_name}/magellan/{experiment_name}/{report_train_name}_{report_test_name}.csv',
              "w") as f:
        f.write(
    ...
    for run in range(1, 4):
        for feature_combination in feature_combinations:
            ...
            for k, v in classifiers.items():
                ...
                labels_train = feats_train['label']
                ...
                if write_test_set_for_inspection:
                    ...
                    out_path = f'../../../data/processed/inspection/{dataset_name}/magellan/{experiment_name}/'
                ...
                dump(model, f'../../../models/{dataset_name}/magellan/{experiment_name}/{report_train_name}_{report_test_name}_{k}_{feature_report}_{run}.joblib')

                with open(f'../../../reports/{dataset_name}/magellan/{experiment_name}/{report_train_name}_{report_test_name}.csv', "a") as f:
...
if __name__ == '__main__':
    ...
    feature_combinations = [['title'], ['brand', 'title'], ['brand', 'title', 'description'],
                            ['brand', 'title', 'description', 'specTableContent']]
    ...
    for file in glob.glob('../../../data/processed/wdc-lspc/magellan/learning-curve/formatted/*'):
        if 'train_' in file and 'pairs' in file and 'metadata' not in file:
            ...
            run_magellan(file, valid, test, feature_combinations, classifiers, experiment_name, dataset_name='wdc-lspc',
                         write_test_set_for_inspection=True)

    # new test exp
    feature_combinations = [['title'], ['brand', 'title'], ['brand', 'title', 'description'],
                            ['brand', 'title', 'description', 'specTableContent']]
    experiment_name = 'new-testset'
    train_sizes = ['xlarge', 'large', 'medium', 'small']

    for train_size in train_sizes:

        file = f'../../../data/processed/wdc-lspc/magellan/learning-curve/formatted/preprocessed_computers_train_{train_size}_magellan_pairs_formatted.csv'
        valid = file.replace('train_', 'valid_')

        test = '../../../data/processed/wdc-lspc/magellan/new-testset/formatted/preprocessed_computers_new_testset_1500_magellan_pairs_formatted.csv'
