...
from model.loss import nll_loss, cross_entropy_loss, LabelSmoothingCrossEntropy
...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, inputs in enumerate(self.data_loader):
            data, token_ids, attn_mask, target = inputs['input_ids'].to(self.device), inputs['token_type_ids'].to(self.device), inputs['attention_mask'].to(self.device), inputs['labels'].to(self.device)
...
class TrainerJoint(BaseTrainer):
    ...
    def __init__(self, model, criterion, metric_ftns, optimizer, config, data_loader,
        ...
        multi_labels1 = [v['label_multi1'] for v in self.data_loader.dataset.data]
        multi_labels1 = pd.Series(multi_labels1)
        ...
        multi_labels2 = [v['label_multi2'] for v in self.data_loader.dataset.data]
        multi_labels2 = pd.Series(multi_labels2)
...
class DistilTrainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        acc_loss, acc_output, acc_target = 0.0, None, None
        for batch_idx, inputs in enumerate(self.data_loader):
            data, attn_mask, target = inputs['input_ids'].to(self.device), inputs['attention_mask'].to(self.device), inputs['labels'].to(self.device)

            target = target.transpose(0, 1)

            with autocast():
                output = self.model(data, attn_mask)
                if self.config.config.get('pos_neg_ratio'):
                    loss = self.criterion(output, target, self.config['pos_neg_ratio'])
                else:
                    loss = self.criterion(output, target)
